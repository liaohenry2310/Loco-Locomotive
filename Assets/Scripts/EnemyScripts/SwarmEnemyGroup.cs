...
public class SwarmEnemyGroup : Enemy
{
        ...
        switch (mCurrentState)
        {
            case State.Attack:
                if (swarmNeighbors.Count != 0)
                {
                    ...
                    Vector3 aimmingPos;
                    if (swarmNeighbors[0] != null)
                    {
                        if (!spawnSelected)
                        {
                            currentEnemy = swarmNeighbors[0].gameObject;
                            aimmingPos = currentEnemy.GetComponent<SwarmEnemy>().UpdatePos(targetPos);
                            currentEnemy.transform.position += aimmingPos * Time.deltaTime * speed * speed;
                        }
                    }
                    else
                    {
                        spawnSelected = true;
                        swarmNeighbors.RemoveAt(0);
                    }
    ...
    public void SpawnGroup()
    {
        ...
        {
            enemy = Instantiate(swarmType, Vector3.zero,   //size of circle base on size of agents , so not gonna be so far apart that they are all flying away from each other.
